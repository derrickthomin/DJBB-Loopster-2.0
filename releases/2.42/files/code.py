...
def process_notes(notes, is_on, record=True, playback_pad_idx=None):
    ...
    for note in notes:
        ...
        # Get output channel based on pad's channel setting
        # For loop playback: use playback_pad_idx with recorded event_channel
        # For live presses: use padidx with no recorded channel (As Recorded → global)
        if playback_pad_idx is not None:
            output_channel = midi.get_midi_channel_for_pad(playback_pad_idx, event_channel)
        else:
            output_channel = midi.get_midi_channel_for_pad(padidx, event_channel)
...
def process_cc_events(cc_events, record=True, loop_pad_idx=C.DEFAULT_LOOP_PAD_IDX, loop_type=None):
    ...
    for cc_event in cc_events:
        ...
        # Use pad's channel setting with recorded event_channel
        output_channel = midi.get_midi_channel_for_pad(loop_pad_idx, event_channel)
...
def process_loop_notes():
    ...
    for idx, loop in enumerate(loop_manager.loops):
        if loop == "" or not loop.loop_is_playing:
            continue
        if s.midi_sync and loop.playback_use_clock and not loop_manager.play_queue[idx]:
            continue
