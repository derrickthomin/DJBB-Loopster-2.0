...
class LoopManager:
    ...
    def __init__(self):
        self.loops = [""] * C.NUM_PADS    # Stores loop objects for pads
    ...
    def add_remove_loop(self, pad_idx):
        # Add
        if self.loops[pad_idx] == "" and self.recording_pad is None:
            self._create_new_loop(pad_idx)

        # Switch recording pad
        elif self.loops[pad_idx] == "" and self.recording_pad is not None and self.recording_pad != pad_idx:
            current_loop = self.loops[self.recording_pad]
            if not current_loop.has_events():
                self._remove_loop(self.recording_pad)
            else:
                self.total_events_count += current_loop.count_events()
                current_loop.toggle_record_state(False)
                pixels.set_blink(self.recording_pad, False)
            self._create_new_loop(pad_idx)

        # Remove
        elif self.recording_pad == pad_idx or (not self.is_recording):
            self._remove_loop(pad_idx)

    def _create_new_loop(self, pad_idx):
        ...
        self.loops[pad_idx] = make_midi_loop(
        ...
        pixels.set_default_color(pad_idx, C.LOOP_COLOR)
        ...
        if settings.midi_sync and not clock.is_playing:
            ...
            self.loops[pad_idx].toggle_record_state()
    ...
    def _remove_loop(self, pad_idx):
        if self.loops[pad_idx] != "":
            removed_events = self.loops[pad_idx].count_events()
            self.total_events_count -= removed_events
        
        self.loops[pad_idx].clear()
        self.loops[pad_idx] = ""
        self.recording_pad = None
    ...
    def check_event_limits(self):
        if self.recording_pad is None:
            return
        loop = self.loops[self.recording_pad]
    ...
    def handle_fn_press(self, action_type="press"):
        ...
        if action_type == "press" and self.is_recording:
            ...
            if current_loop.loop_type == "hold":
                self.loops[pad_idx].toggle_playstate(False)
                pixels.set_blink(pad_idx, False)
            ...
            if settings.midi_sync:
                ...
                if clock.is_playing:
                    self.loops[pad_idx].toggle_playstate(True)
                ...
                if not self.loops[pad_idx].loop_is_playing:
                    self.loops[pad_idx].toggle_playstate(True)
    ...
    def change_loop_mode(self, button_idx, forward=True):
        """Toggle between oneshot and loop mode for the loop."""
        if self.loops[button_idx] != "":
            loop_type = self.loops[button_idx].change_loop_mode(forward=forward)
            self.loops[button_idx].clear_notes_and_pixels()
            self.loops[button_idx].toggle_playstate(False)
            self.play_queue[button_idx] = False
            pixels.set_blink(button_idx, False)
            self.display_loop_mode(button_idx)
            if loop_type != settings.loop_type:
                settings.loop_type = loop_type

    def display_loop_mode(self, idx):
        if self.loops[idx] != "":
            display.show_notification(f"Loop Type: {self.loops[idx].loop_type}")

    def toggle_loop_playstate(self, idx, force_play=False, force_stop=False):
        """Handle pad press to toggle loop play/stop.
        ...
        """
        if self.is_recording or not self.loops[idx]:  # This press is for a note if recording
            return
        ...
        if force_stop:
            self.loops[idx].toggle_playstate(False)
        ...
        if settings.midi_sync:
            if self.loops[idx].loop_type == "oneshot":
    ...
    def start_armed_recording(self):
        ...
        self.recording_is_armed = False
        self.loops[self.recording_pad].toggle_record_state()
    ...
    def stop_all_loops(self):
        ...
        for idx in range(C.NUM_PADS):
            ...
            if loop_obj:
                self._stop_single_loop(idx, loop_obj)

    def _stop_single_loop(self, idx, loop_obj):
        if self.play_queue[idx] and loop_obj.loop_is_playing:
            pixels.set_blink(idx, True, C.PIXEL_LOOP_PLAYING_COLOR)
        else:
            self.play_queue[idx] = False
        
        loop_obj.toggle_playstate(False)
    ...
    def _play_loop(self, idx, on_or_off=None, align_to_clock=True, use_midi_clock=None):
        ...
        if self.loops[idx].loop_type == "loop":
            ...
            if previous_state and not self.loops[idx].loop_is_playing:
                self.loops[idx].clear_notes_and_pixels()
                pixels.set_default_color(idx, C.LOOP_COLOR)

            # Now On
            elif not previous_state and self.loops[idx].loop_is_playing:
                self.loops[idx].clear_notes_and_pixels()
                pixels.set_default_color(idx, C.PIXEL_LOOP_PLAYING_COLOR)
                pixels.set_color(idx, C.PIXEL_LOOP_PLAYING_COLOR)
        else:
    ...
    def unique_notes(self, padidx):
        if self.loops[padidx] != "":
            return self.loops[padidx].get_unique_notes()
        return []
    
    def unique_ccs(self, padidx):
        if self.loops[padidx] != "":
            return self.loops[padidx].get_unique_ccs()
        return []
    ...
    def load_loops_binary(self, loops_metadata):
        ...
        for pad_idx_str, meta in loops_metadata.items():
            ...
            loop.has_loop = True
            self.loops[pad_idx] = loop
            pixels.set_default_color(pad_idx, C.LOOP_COLOR)
            self.finalize_loop_load(pad_idx)
    ...
    def update_pad_pixels(self):
        for pad_idx, loop in enumerate(self.loops):
            if loop == "":
                pixels.set_default_color(pad_idx)
    ...
    def finalize_loop_load(self, pad_idx):

        if pad_idx is None or self.loops[pad_idx] == "":
            return
        
        # Update total events counter for the loaded loop
        loaded_events = self.loops[pad_idx].count_events()
        self.total_events_count += loaded_events
        
        self.loops[pad_idx].create_oneshot_ccs()
        
        # Only generate oneshot notes if needed (oneshot mode + all-at-once setting)
        # Otherwise, lazy generation will happen at playback time if needed
        if self.loops[pad_idx].loop_type == "oneshot" and settings.notes_all_at_once:
    ...
    def handle_midi_sync_change(self):
        """Stop all loops and clear play queue when MIDI sync mode changes."""
        # MIDI sync disabled - stop all playing loops and clear play queue
        for idx in range(C.NUM_PADS):
            loop_obj = self.loops[idx]
            if loop_obj and loop_obj != "":
                ...
                # Reset pixel colors to default loop color
                pixels.set_blink(idx, False)
                pixels.set_default_color(idx, C.LOOP_COLOR)
                pixels.set_color(idx, C.LOOP_COLOR)
        
        # Reset any global loop playing state
        self.any_loop_playing = False
